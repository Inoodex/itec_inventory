...
<div class="content container-fluid">
    ...
    <div class="page-header mb-4">
        <div class="content-page-header d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <h4 class="card-title fw-bold text-dark mb-1">Edit Purchase Order #{{ $purchaseNo }}</h4>
                <p class="text-muted small mb-0">Modify supplier items, unit costs, quantities, serials, and financial breakdown</p>
            </div>
            <div>
    ...
    <form method="POST" action="{{ route('purchase.update', $purchase->id) }}" id="purchaseForm">
            ...
            <div class="card-body p-4">
                ...
                <div class="row g-3">
                        ...
                        <select name="vendor_id" id="vendor_id" class="form-select select2 border-light-subtle" required>
                            ...
                            @foreach ($vendors as $vendor)
                                <option value="{{ $vendor->id }}" {{ (old('vendor_id', $vendorId) == $vendor->id) ? 'selected' : '' }}>
                                    {{ $vendor->name }} @if(!empty($vendor->company_name)) ({{ $vendor->company_name }}) @endif
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-lg-3 col-md-4 col-12">
                        ...
                        <input type="date" name="purchase_date" class="form-control border-light-subtle" value="{{ old('purchase_date', $purchaseDate) }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-3 mb-4">
            <div class="card-body p-4">
                <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                    <h6 class="fw-bold text-dark mb-0"><i class="fe fe-shopping-cart me-2 text-primary"></i>Purchased Items Cart</h6>
                ...
                <div class="form-section-box p-3 bg-white rounded-3 mb-4 border shadow-sm">
                    <div class="row align-items-center g-2">
                        ...
                        <div class="col">
                            ...
                            <input type="text" id="purchase_barcode_scanner" class="form-control form-control-lg border-light-subtle font-monospace" placeholder="Scan product barcode and press Enter..." autocomplete="off">
                        </div>
                ...
                <div class="form-section-box p-3 bg-light rounded-3 mb-4 border" id="product-builder">
                    <div class="row g-3 align-end">
                        <div class="col-lg-4 col-md-6 col-12">
                            <label class="form-label small text-secondary fw-semibold mb-1">Select Product to Add</label>
                        ...
                        <div class="col-lg-2 col-md-3 col-6">
                            <label class="form-label small text-secondary fw-semibold mb-1">Unit Cost Price</label>
                            <input type="number" step="0.01" id="builder_unit_price" class="form-control border-light-subtle" placeholder="0.00">
                        </div>

                        <div class="col-lg-2 col-md-3 col-6">
                            <label class="form-label small text-secondary fw-semibold mb-1">Quantity</label>
                ...
                <div id="empty-cart-msg" class="text-center py-4 text-muted d-none">
                    <i class="fe fe-shopping-cart fs-1 text-light"></i>
                    <p class="mt-2 mb-0">No items in cart. Please select a product above to add.</p>
                </div>
            </div>
        </div>

        <!-- Section 3: Financial Summary -->
        <div class="card border-0 shadow-sm rounded-3 mb-4" id="summarySection">
            <div class="card-body p-4">
                ...
                <div class="row g-3">
                    <div class="col-lg-3 col-md-6 col-12">
                        ...
                        <input type="number" step="0.01" id="subTotal" name="sub_total" class="form-control bg-light border-light-subtle" readonly value="{{ number_format($subTotal, 2, '.', '') }}">
                    </div>
                    <div class="col-lg-3 col-md-6 col-12">
                        <label class="form-label small text-secondary fw-semibold mb-1">Discount</label>
                        <input type="number" step="0.01" id="discount" name="discount" class="form-control border-light-subtle" value="{{ number_format($discount, 2, '.', '') }}" oninput="calculateGrandTotal()">
                    </div>
                    <div class="col-lg-3 col-md-6 col-12">
                        <label class="form-label small text-secondary fw-semibold mb-1">Grand Total (Payable)</label>
                        <input type="number" step="0.01" id="grandTotal" name="grand_total" class="form-control bg-light border-light-subtle fw-bold" readonly value="{{ number_format($totalAmount, 2, '.', '') }}">
                    </div>
                    <div class="col-lg-3 col-md-6 col-12">
                        <label class="form-label small text-secondary fw-semibold mb-1">Payment Amount</label>
                        <input type="number" step="0.01" id="paymentAmount" name="payment" class="form-control border-light-subtle" value="{{ number_format($payment, 2, '.', '') }}" oninput="calculateGrandTotal()">
                    </div>
                    <div class="col-lg-3 col-md-6 col-12">
                        <label class="form-label small text-secondary fw-semibold mb-1">Due Amount</label>
                        <input type="number" step="0.01" id="dueAmount" name="due" class="form-control bg-light border-light-subtle text-danger fw-bold" readonly value="{{ number_format($due, 2, '.', '') }}">
                    </div>
                </div>
        ...
        <div class="card border-0 shadow-sm rounded-3 mb-4">
            <div class="card-body p-4 d-flex justify-content-end gap-2">
                ...
                <button type="submit" id="submitBtn" class="btn btn-primary px-4 py-2 rounded-3 shadow-sm">
...
document.addEventListener('DOMContentLoaded', function() {
    // Initial pre-loaded cart items from server
    let cartItems = {!! json_encode($cartItems) !!} || [];
    let itemCounter = cartItems.length;
    let currentSerials = [];

    // Ensure numeric fields and IDs are properly formatted
    cartItems = cartItems.map(function(item, idx) {
        return {
            id: idx + 1,
    ...
    function renderCart() {
        ...
        cartItems.forEach(function(item, idx) {
            var serialDisplay = item.serials && item.serials.length > 0
    ...
    window.calculateGrandTotal = function() {
        ...
        var due = Math.max(0, grandTotal - payment);
    ...
    // Initial render and calculation of pre-loaded items
    renderCart();
    calculateGrandTotal();
